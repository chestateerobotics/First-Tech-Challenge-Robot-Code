...
public class LeftTurnAutoNew extends LinearOpMode
{
    ...
    public void runOpMode() {
        ...
        TrajectorySequence startMove = drive.trajectorySequenceBuilder(new Pose2d(0, 0, Math.toRadians(90)))
                ...
                .splineTo(new Vector2d(0, 52.5), Math.toRadians(135))
                .splineTo(new Vector2d(-3.5355339059, 52.5+3.5355339059), Math.toRadians(135))
                .build();
        ...
        TrajectorySequence parkMiddle = drive.trajectorySequenceBuilder(back1.end())
                ...
                .lineToConstantHeading(new Vector2d(-1, 46.5))
                .build();
    ...
    private void encoderArm(int level, double power){
        if(power != 0.0) {
            if (level == 0) {
                ...
                rightLift.setTargetPosition(350-encoderSubtracter);
                leftLift.setTargetPosition(350-encoderSubtracter);
                rightLift.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                leftLift.setMode(DcMotor.RunMode.RUN_TO_POSITION);
            }
